...
[UpSetR](https://cran.r-project.org/web/packages/UpSetR/readme/README.html) is a package for visualising the intersections of many more sets than is feasible with, for example, Venn diagrams. In my research, I find these plots extremely powerful for showing large amounts of information in an attractive and intuitive way, with lots of options for exploring the data.  
...
For this, I'm going to reshape it back into wide format, but this time each row will correspond to a player, with a 0 for a tournament they have won, or a 0 for tournaments they haven't, as well as the tour (Mens or Womens) and the total number of slams they have won.
...
While Venn diagrams are useful for describing overlaps between sets, they are limited in the number of sets they use, and the reliance on numbers can make patterns tricky to spot. 
...
* `sets.bar.color`: change the colour of the bars on the left, which are visualising the number of winners of each Slam
